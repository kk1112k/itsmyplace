	...
	<tbody>
		<c:if test="${!empty list}">
			<c:forEach var="comm" items="${list}" varStatus="status">
				<tr>
					<td class="text-center"><c:out value="${comm.bbsSeq}" /></td>
					<td>
						<a href="javascript:void(0)" onclick="fn_view(${comm.bbsSeq})">
							<c:out value="${comm.bbsTitle}" />
						</a>
					</td>
					<td class="text-center"><c:out value="${comm.userId}" /></td>
					<td class="text-center">${comm.regDate}</td>
					<td class="text-center"><fmt:formatNumber type="number" maxFractionDigits="3" value="${comm.bbsReadCnt}" /></td>
				</tr>
			</c:forEach>
		</c:if>
	</tbody>
